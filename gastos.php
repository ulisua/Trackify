...
$stmt_totales = $conn->prepare("SELECT SUM(monto) as total, COUNT(id_movimiento) as cantidad FROM movimientos WHERE id_usuario = ? AND tipo = 'gasto' AND fecha BETWEEN ? AND ?");
...
$stmt_lista = $conn->prepare("SELECT m.id_movimiento, m.fecha, m.descripcion, m.monto, m.id_categoria, c.nombre as categoria_nombre FROM movimientos m JOIN categorias c ON m.id_categoria = c.id_categoria WHERE m.id_usuario = ? AND m.tipo = 'gasto' AND m.fecha BETWEEN ? AND ? ORDER BY m.fecha DESC");
                    ...
                    <?php if (count($gastos) > 0): ?>
                        <?php foreach ($gastos as $gasto): ?>
                            <tr id="fila-<?= $gasto['id_movimiento'] ?>">
                                ...
                                <td class="negativo"><span data-ars="<?= $gasto['monto'] ?>" data-prefijo="-">-$<?php echo number_format($gasto['monto'], 2, ',', '.'); ?></span></td>
                                <td>
                    ...
                    <?php foreach ($gastos as $gasto): ?>
                        <div class="movimiento-card" id="card-<?= $gasto['id_movimiento'] ?>">
                            <div class="mc-top">
                                ...
                                <span class="mc-monto negativo" data-ars="<?= $gasto['monto'] ?>" data-prefijo="-">-$<?php echo number_format($gasto['monto'], 2, ',', '.'); ?></span>
                            </div>
                            <div class="mc-bottom">
